...
/** Fan tab moved to apps/fan (HEX-147). */
function FanMigratedNotice() {
  return (
    <div className="mono rounded-md border border-line bg-surface-white p-5 text-sm text-label-muted">
      ...
      <code className="rounded bg-cream px-1.5 py-0.5">pnpm --filter @stadiyums/fan dev</code>{" "}
      to view it.
    </div>
  );
}

/** Runner tab moved to apps/runner (HEX-148). */
function RunnerMigratedNotice() {
  return (
    <div className="mono rounded-md border border-line bg-surface-white p-5 text-sm text-label-muted">
      The runner demo now lives in <b>apps/runner</b>. Run{" "}
      <code className="rounded bg-cream px-1.5 py-0.5">pnpm --filter @stadiyums/runner dev</code>{" "}
      to view it.
    </div>
  );
}
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-[880px] px-5 pb-20 pt-2">
        {activeTab === "fan" ? <FanMigratedNotice /> : <RunnerMigratedNotice />}
      </main>
      {theme.showPoweredBy && (
