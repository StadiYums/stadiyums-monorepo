    ...
    return (
      <div className="space-y-3" aria-label="Loading ready orders" aria-busy="true">
        {[0, 1].map((key) => (
          <div
            key={key}
            className="h-36 animate-pulse rounded-xl border border-line bg-surface-white"
          />
        ))}
      </div>
    );
    ...
    return (
      <div className="rounded-xl border border-line bg-cream p-5">
        ...
        <p className="mt-2 text-sm font-medium leading-relaxed text-label-muted">
          You are listening for Sections 101–115. We will surface the next pickup here.
        </p>
        <p className="mono mt-4 text-[10px] font-bold uppercase tracking-[0.06em] text-green">
          Live · last checked just now
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {displayError ? (
        <p
          role="alert"
          className="rounded-lg border border-orange/30 bg-orange/10 px-4 py-3 text-sm font-semibold text-orange-dim"
        >
          {advanceError
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="mono text-[10px] font-bold uppercase tracking-[0.06em] text-label-muted">
                  Order {order.orderNumber}
                </p>
                <p className="mt-2 font-display text-[1.65rem] leading-none text-navy">
            ...
            <div className="mt-4 flex items-center justify-between gap-3">
              <p className="mono text-[11px] font-bold uppercase tracking-[0.04em] text-orange-dim">
                Ready {elapsed(order.placedAt, now)}
              </p>
              <Button
                variant="advance"
              ...
              >
                {advancingId === order.id ? "Claiming…" : actionLabel(order.status)}
              </Button>
            </div>
          </article>
        );
